...
export default function RoomsPage() {
  ...
  const [availabilityFilter, setAvailabilityFilter] = useState<'all' | 'occupied' | 'vacant'>('all');
  ...
  const filteredRooms = rooms.filter((room) => {
    ...
    const isOccupied = activeTenancy?.status === 'OCCUPIED';

    const matchesSearch =
      searchQuery.trim().length === 0 ||
      room.label.toLowerCase().includes(searchQuery.toLowerCase()) ||
      activeTenancy?.tenant?.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      activeTenancy?.tenant?.email?.toLowerCase().includes(searchQuery.toLowerCase());

    const matchesAvailability =
      availabilityFilter === 'all' ||
      (availabilityFilter === 'occupied' && isOccupied) ||
      (availabilityFilter === 'vacant' && !isOccupied);
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <input
            ...
            placeholder="Search room label, tenant name, or email"
          ...
          <select
            ...
            onChange={(e) => setAvailabilityFilter(e.target.value as 'all' | 'occupied' | 'vacant')}
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-sm"
          >
